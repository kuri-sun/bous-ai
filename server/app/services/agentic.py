...
def _build_agentic_prompt(
    ...
    instructions = (
        ...
        "2) 会話開始時（履歴が空）は必ず質問で始める。質問は具体的な不足情報を1〜3問だけ列挙する。"
        "3) 質問は広い問いではなく、避難経路の地名/ルート、連絡先の番号と役割、備蓄品の種類と数量、要支援者サポート体制などの具体情報を尋ねる。"
        "4) ユーザーの直近の回答が曖昧・不足している場合は、追加の質問。質問は1ターンに1〜3問まで列挙してよい。"
        "5) 情報が十分に揃ったと判断できるときだけ、具体的な改定提案を1つ提示。"
        "6) 提案は2〜4文の短い段落で、箇条書きは使わない。"
        "7) 余計な説明やメタ情報は不要。"
        "8) 以下のテキストを参考に、足りない箇所や改善が必要な箇所にフォーカスすること: "
        "`context.search_reference_text` は公式マニュアルPDFのテキスト, "
        "`context.generated_plain_text` は現在の生成マニュアルの本文。"
...
def build_agentic_turn(
    ...
    return {
        "kind": "question",
        "content": "防災マニュアルを改善するために、補足したい情報があれば教えてください。",
    }
