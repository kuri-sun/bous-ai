...
export default function SessionPage() {
  ...
  const loadSessions = async (force = false) => {
    if (!force) {
      try {
        ...
        if (cached) {
          const data = JSON.parse(cached) as SessionsResponse;
    ...
    try {
      ...
      setSessions(data.sessions);
      try {
        sessionStorage.setItem("sessions-cache", JSON.stringify(data));
      } catch {
  ...
  const loadSessionDetail = async (id: string) => {
    try {
      ...
      const inputs = (session.inputs ?? {}) as Record<string, any>;
      const step1 = (inputs.step1 ?? {}) as Record<string, any>;
      setTextInput(step1.memo ?? "");
      setFileDescription(step1.file_description ?? "");
      setFile(null);

      const step1Extracted = (inputs.step1_extracted ?? null) as Record<
        string,
        unknown
      > | null;
      if (session.form && session.msg) {
        setAnalyzeResult({
          msg: session.msg,
          form: session.form,
          extracted: step1Extracted ?? undefined,
        });
        const step2 = (inputs.step2 ?? {}) as Record<string, string>;
        const initialAnswers: Record<string, string> = {};
        session.form.fields.forEach((field) => {
          initialAnswers[field.id] = step2[field.id] ?? "";
        });
        setAnswers(initialAnswers);
        setStep(session.status === "step2" ? 2 : 1);
      } else {
    ...
    <main className="flex h-full w-full text-emerald-950">
      <div className="h-full w-full">
        <div className="grid h-full grid-cols-5">
          <aside className="col-span-1 flex h-full flex-col border-r border-emerald-100">
                ...
                sessions.map((session) => (
                  <button
                    ...
                    className={`w-full px-3 py-2 text-left text-sm hover:bg-emerald-50 ${
                      sessionId === session.id ? "bg-emerald-50" : ""
                    }`}
                  >
